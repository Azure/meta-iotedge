...
def _is_registry_source(source: str) -> bool:
    """Check if a Cargo.lock source string is a registry (not git/path).

    Matches registry+, sparse+, and any other non-git/path sources that
    carry a checksum (e.g. future Cargo registry protocols).
    """
    if not source:
        return False
    return not (source.startswith("git+") or source.startswith("path+"))


def parse_cargo_lock(lockfile_path: str) -> list:
    """Parse Cargo.lock and return list of (name, version, checksum) for registry crates."""
    crates = []
    with open(lockfile_path, encoding="utf-8") as fh:
        name = version = checksum = source = None
        for line in fh:
            line = line.strip()
            if line == "[[package]]":
                if name and version and checksum and _is_registry_source(source):
...
def cmd_generate_crates_inc(lockfile_path: str, output_path: str):
    """Generate a *-crates.inc file from a Cargo.lock file.
    
    This produces the same format as 'bitbake -c update_crates':
    - SRC_URI with crate:// entries
    - SRC_URI[name-version.sha256sum] entries
    """
    crates = parse_cargo_lock(lockfile_path)
    if not crates:
        print(f"Warning: No registry crates found in {lockfile_path}", file=sys.stderr)
        placeholder_lines = [
            "# Auto-generated by update-recipes.sh - do not edit manually\n",
            "# No registry crates found in Cargo.lock when this file was generated.\n",
            "# Use \"bitbake <recipe> -c update_crates\" to update when crates are available.\n",
        ]
        with open(output_path, "w", encoding="utf-8") as fh:
            fh.writelines(placeholder_lines)
        print(f"  Generated {output_path} (0 crates)")
        return

    lines = []
    lines.append("# Auto-generated by update-recipes.sh - do not edit manually\n")
    lines.append("# Use \"bitbake <recipe> -c update_crates\" to update\n")
    lines.append("\n")
    ...
    for name, version, checksum in crates:
        key = f"{name}-{version}"
        lines.append(f'SRC_URI[{key}.sha256sum] = "{checksum}"\n')

    with open(output_path, "w", encoding="utf-8") as fh:
        fh.writelines(lines)
    print(f"  Generated {output_path} ({len(crates)} crates)")
...
COMMANDS = {
    ...
    "generate-crates-inc": (cmd_generate_crates_inc, ["lockfile_path", "output_path"]),
}
